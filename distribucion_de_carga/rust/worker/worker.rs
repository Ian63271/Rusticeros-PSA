...
async fn main() {
    ...
    // 1. Leemos los argumentos de la terminal
    let args: Vec<String> = env::args().collect();
    
    // 2. Si nos pasan un puerto (ej: cargo run 3001), lo usamos. 
    // Si no, usamos el 3000 por defecto.
    let port: u16 = if args.len() > 1 {
        args[1].parse().expect("El puerto debe ser un número válido")
    } else {
        3000
    };

    // 3. Usamos la variable 'port' en lugar del 3000 fijo
    let addr = SocketAddr::from(([0, 0, 0, 0], port));

    println!("Worker iniciado y escuchando en http://{}", addr);
